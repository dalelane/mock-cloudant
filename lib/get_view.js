...
module.exports = function (self) {
  ...
  return function (req, res, next) {
    ...
    // if the mapper function has been stored as a string, we need to convert it back into a function before we can call it
    if (view.map && typeof view.map === 'string' && view.map.startsWith('function (doc) {')) {
      view.map = new Function('doc', view.map.slice('function (doc) {'.length, -1));
    }  
